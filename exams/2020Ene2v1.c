    ...
    main(int argc, char *argv[]){
        ...
        while(1) {
            ...
            if((s = getnameinfo((struct sockaddr *) &addr, addrlen, host, NI_MAXHOST, service, NI_MAXSERV, NI_NUMERICSERV)) == -1) handle_error_gai(s);

            printf("Received msg from %s:%s\n", host, service);

            time_t t=time(NULL);
            strftime(out,BUF_SIZE,"%H:%M:%S",localtime(&t));
