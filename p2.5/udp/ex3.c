...
#define USAGE  "ERROR: Usage: %s [address] [port] [command]\n"
...
int main(int argc,char **argv){
    ...
    if( (sendto(e2, argv[3], sizeof(argv[3]), 0, dest_addr->ai_addr, dest_addr->ai_addrlen)) == -1) handle_error("Error in sendto()");
    
    addrlen = sizeof(src_addr);

    freeaddrinfo(dest_addr); //no longer needed

    if( (bytes_read = recvfrom(e2, msg, MESSAGE_MAXSIZE, 0, (struct sockaddr *) &src_addr, &addrlen)) == -1) handle_error("Error reading from client");
