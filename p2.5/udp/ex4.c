...
int main(int argc,char **argv){
    ...
    do{
        ...
        if ( (retval = select(e2+1, &setfd, NULL, NULL, NULL))== -1) handle_error("Error in slect()");
           
        if(FD_ISSET(e2, &setfd)){
            printf("Command received via network\n");
        ...
        if(FD_ISSET(STDIN_FILENO, &setfd)){
            printf("Command received via terminal\n");

            if((bytes_read = read(STDIN_FILENO, buf, 2)) == -1) handle_error("Error reading from input");
        ...
        if(FD_ISSET(STDIN_FILENO, &setfd)){
            write(STDIN_FILENO, msg, bytes_write);
        } 
